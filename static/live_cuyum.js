...
function escapeHtml(value) {
  ...
  function firstRelevantCell(data) {
    ...
    return cells.find(c => Number(c.confirming || c.estaciones_confirmando || 0) > 0)
  ...
  function statusFromData(data) {
    ...
    const text = cleanText(`${display.status || ''} ${data.display_status || ''} ${display.message || ''} ${data.display_message || ''} ${data.mensaje || ''} ${data.nivel || ''}`);
    const isSound = alert.sound || data.sonar || Number(data.buzzer_segundos || 0) > 0;

    if (isSound || text.includes('anticip') || text.includes('señal')) {
    ...
    if (text.includes('confirm') || text.includes('movimiento posible') || Number(data.estaciones_confirmando || 0) >= 2) {
      return { mode: 'watch', word: 'Movimiento posible', hint: `${cellHumanLabel(cell)} · datos coincidentes` };
    }
    if (text.includes('observ') || text.includes('senal') || text.includes('señal') || Number(data.estaciones_confirmando || 0) === 1) {
...
function displayCellId(cell, i) {
  ...
  function setStateCard(data) {
    ...
    const sensors = network.sensors_active ?? data.sensores_activos ?? '--';
